        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center">
            ...
            <div className="flex items-center">
              ...
              <h1 className="text-xl sm:text-2xl md:text-3xl font-bold bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent">Daily Study Schedule</h1>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-6 max-w-4xl mx-auto">
          {daysWithSessions.length > 0 ? (
            daysWithSessions.map(([date, sessions], dayIndex) => {
              return (
                <Card key={date} className="p-8 bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm border-0 shadow-2xl">
                  <div className="flex items-center justify-between mb-6">
                    <div className="flex items-center space-x-4">
                      ...
                      <div>
                        <h3 className="text-xl font-bold text-gray-900 dark:text-white">{getDayNameFromDate(date)}</h3>
                  ...
                  <div className="space-y-4">
                    {(sessions as any[]).map((session) => (
                      <div key={session.id} className={`flex items-center justify-between p-6 border border-gray-200 dark:border-gray-700 rounded-xl transition-all duration-300 ${session.completed
                        ...
                        <div className="flex items-center space-x-4">
                          <div className="flex items-center space-x-3">
                            <div className={`w-10 h-10 rounded-xl flex items-center justify-center shadow-lg ${session.completed
                              ...
                              <span className={`font-semibold text-lg ${session.completed ? 'line-through text-gray-500 dark:text-gray-400' : 'text-gray-900 dark:text-white'}`}>
                                {session.subjects?.name || "Deleted Subject"}
                              </span>
                            
                          </div>
                          <Badge className={`px-3 py-1 rounded-full text-xs font-medium ${getPriorityColor(session.subjects?.priority)}`}>
                            {session.subjects?.priority || "N/A"}
                          </Badge>
                        </div>
            ...
            <Card className="p-12 text-center bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm border-0 shadow-2xl">
              <div className="space-y-6">
                ...
                <div>
                  <h3 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">No Study Sessions</h3>
                  <p className="text-gray-600 dark:text-gray-400">All your study sessions have been completed or removed.</p>
                </div>
                <Link to="/add-subject">
                  <Button className="bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 text-white px-6 py-3 mt-4 font-semibold rounded-lg shadow-lg hover:shadow-xl transition-all duration-200">
